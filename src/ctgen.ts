...
program
  .name("ctgen")
  .description(
    "CLI for generating structured project scaffolds (Next.js, .NET)"
  )
  .version("1.0.0");
...
program
  ...
  .option(
    "-p, --path <path>",
    "Path to create/find the project directory (default: current directory)"
  )
  .option(
    "-n, --name <name>",
    "Name of the project (used for folder creation or lookup)"
  )
  .action(
    async (
      ...
      try {
          ...
          const nameAnswer = await inquirer.prompt([
            {
              ...
              validate: (input: string) =>
                !!input || "Project name is required",
            },
          ]);
        ...
        let opts: ScaffoldNextOptions = { outputDir, ...options };
        if (options.all) {
          opts = {
            ...opts,
            withMdx: true,
          ...
          let answers = {};
          if (prompts.length > 0) {
            answers = await inquirer.prompt(prompts as any[]);
            opts = { ...opts, ...answers };
          }
          ...
          if (opts.withPrisma) {
            ...
            const dbAnswers = await inquirer.prompt(dbPrompts as any[]);
            opts = { ...opts, ...dbAnswers };
            if (dbAnswers.dbMode === "new") {
              const newDbPrompts: any[] = [
                {
                  type: "input",
                  name: "dbName",
              ...
              const newDbAnswers = await inquirer.prompt(newDbPrompts as any[]);
              opts = { ...opts, ...newDbAnswers };
            } else {
              const existingDbPrompt: any[] = [
                {
                  type: "input",
                  name: "databaseUrl",
  ...
  .action(
    async (
      ...
      try {
        ...
        const outputDir = path.join(projectPath, projectName);
        let opts: ScaffoldDotnetOptions = { outputDir, ...options };
        if (options.all) {
          opts = {
            ...opts,
            apiType: "WebAPI",
          ...
          for (const prompt of featurePrompts) {
            if (typeof (opts as any)[prompt.name] === "undefined") {
              prompts.push(prompt);
            }
          }
          let answers = {};
