...
async function writeDotnetFile(filePath: string, content: string) {
...
async function getExtensionFiles(options: ScaffoldDotnetOptions) {
...
async function writeBoilerplateFiles(
  outputDir: string,
  options: ScaffoldDotnetOptions
) {
  // EFCore: MyDbContext and WeatherForecast model
  if (options.withEFCore) {
    await writeDotnetFile(
      path.join(outputDir, "Data/Context/MyDbContext.cs"),
...
namespace Data.Context {
  ...
  if (options.apiType === "WebAPI") {
    await writeDotnetFile(
      path.join(outputDir, "Controllers/AIModelsController.cs"),
...
namespace Controllers {
  ...
  if (options.withAuth) {
    await writeDotnetFile(
      path.join(outputDir, "Models/User.cs"),
      `namespace Models {
    public class User {
    ...
    await writeDotnetFile(
      path.join(outputDir, "Controllers/AuthController.cs"),
      `using Microsoft.AspNetCore.Mvc;
...
namespace Controllers {
  ...
  await writeDotnetFile(
    path.join(outputDir, "Models/AIModel.cs"),
    `namespace Models {
  ...
  await writeDotnetFile(
    path.join(outputDir, "Services/Interfaces/IAIModelService.cs"),
  ...
  await writeDotnetFile(
    path.join(outputDir, "Services/Implementations/AIModelService.cs"),
    `using Models;
using Data.Context;
using Services.Interfaces;
...
namespace Services.Implementations {
  ...
  // Copy DI extension
  await writeDotnetFile(
    path.join(outputDir, "Extensions/ServiceCollectionExtensions.cs"),
    generateServiceCollection(options)
  );

  // Write wwwroot/index.html using utility
  await writeDotnetFile(
    path.join(outputDir, "wwwroot/index.html"),
    generateIndexHtml()
  );

  // --- General-use Enums ---
  await writeDotnetFile(
    path.join(outputDir, "Models/Enums/StatusCode.cs"),
  ...
  await writeDotnetFile(
    path.join(outputDir, "Models/Enums/UserRole.cs"),
    `namespace Models.Enums {
  ...
  await writeDotnetFile(
    path.join(outputDir, "Services/Interfaces/IResponse.cs"),
    `namespace Services.Interfaces {
  ...
  await writeDotnetFile(
    path.join(outputDir, "Models/Responses/BaseResponse.cs"),
  ...
  await writeDotnetFile(
    path.join(outputDir, "Models/Responses/ErrorResponse.cs"),
    `namespace Models.Responses {
  ...
  await writeDotnetFile(
    path.join(outputDir, "Models/Responses/SuccessResponse.cs"),
    `namespace Models.Responses {
  ...
  await writeDotnetFile(
    path.join(outputDir, "Models/Responses/ItemResponse.cs"),
    `namespace Models.Responses {
  ...
  await writeDotnetFile(
    path.join(outputDir, "Models/Responses/ItemsResponse.cs"),
...
namespace Models.Responses {
  ...
  await writeDotnetFile(
    path.join(outputDir, ".editorconfig"),
...
`
  ...
  await writeDotnetFile(
    path.join(outputDir, "Docs/Swagger/swagger.json"),
    `{
  "openapi": "3.0.1",
  "info": {
    "title": "My API",
  ...
  await writeDotnetFile(
    path.join(outputDir, "Tests/ServiceTests/AIModelServiceTests.cs"),
    `using Xunit;
using Models;
...
namespace Tests.ServiceTests {
  ...
  await writeDotnetFile(
    path.join(outputDir, "Tests/API/AuthControllerTests.cs"),
    `using Xunit;
using Controllers;
using Models;
using Microsoft.AspNetCore.Mvc;
...
`
  ...
  if (options.withHealthChecks) {
    await writeDotnetFile(
      path.join(outputDir, "Controllers/HealthController.cs"),
      `using Microsoft.AspNetCore.Mvc;
...
namespace Controllers {
  ...
  if (options.withApiVersioning) {
    await writeDotnetFile(
      path.join(outputDir, "Controllers/VersionedController.cs"),
      `using Microsoft.AspNetCore.Mvc;
...
namespace Controllers {
  ...
  await writeDotnetFile(
    path.join(outputDir, ".github/workflows/dotnet.yml"),
    `name: .NET CI
on:
  push:
    branches: [ main ]
...
export async function scaffoldDotnetStructure(
  options: ScaffoldDotnetOptions
): Promise<void> {
  ...
  try {
    ...
    console.log(
      `✅ .NET ${options.apiType} structure scaffolded successfully in ${outputDir}.`
    );
  } catch (error: unknown) {
