...
export function generateProgramCs(options: ScaffoldDotnetOptions) {
  const usings = [
  ...
  if (options.withSwagger) builderSetup += "services.AddCustomSwagger();\n";
  if (options.withAuth)
    builderSetup += "services.AddCustomAuth(builder.Configuration);\n";
  if (options.withCors) builderSetup += "services.AddCustomCors();\n";
  ...
  if (options.withSwagger) appSetup += "app.UseCustomSwagger();\n";
  if (options.withCors) appSetup += "app.UseCustomCors();\n";
  if (options.withAuth) appSetup += "app.UseCustomAuth();\n";
  if (options.withHealthChecks) appSetup += "app.UseCustomHealthChecks();\n";
  appSetup +=
