...
export async function scaffoldNextStructure(options) {
    ...
    try {
        ...
        if (options.withNextAuth) {
            ...
            const writtenMiddleware = await writeFileSafely(middlewarePath, middlewareContent);
            if (!writtenMiddleware)
                skippedFiles.push(middlewarePath);
            // lib/auth/auth.ts
            ...
            if (!writtenAuthTs)
                skippedFiles.push(authTsPath);
            // app/api/auth/[...nextauth]/route.ts
            const routeTsPath = path.join(outputDir, "app/api/auth/[...nextauth]/route.ts");
...
`;
            ...
            if (!writtenSchema)
                skippedFiles.push(schemaPath);
            // Handle database creation and .env
            let databaseUrl = "";
            if (options.dbMode === "new" && options.dbType === "postgres") {
                const { dbName, dbUser, dbPassword, dbHost, dbPort } = options;
                try {
                    ...
                    console.warn(`⚠️ Could not create database automatically. Please ensure Postgres is running and you have permissions, or create the database manually.`);
                }
...
`;
            const writtenTailwindCss = await writeFileSafely(tailwindCssPath, tailwindCssContent);
            if (!writtenTailwindCss)
                skippedFiles.push(tailwindCssPath);
            const writtenTailwindConfig = await writeFileSafely(tailwindConfigPath, tailwindConfigContent);
            if (!writtenTailwindConfig)
                skippedFiles.push(tailwindConfigPath);
        }
        ...
        console.log(`✅ Next.js structure scaffolded successfully in ${outputDir}.`);
        if (skippedFiles.length > 0) {
            // Print skipped files in yellow
            const yellow = "\x1b[33m";
            const reset = "\x1b[0m";
            console.log(`${yellow}The following files already existed and were not created:${reset}`);
